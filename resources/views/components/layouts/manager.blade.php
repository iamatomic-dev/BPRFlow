...
<head>
    ...
    <title>{{ $title ?? 'Dashboard Manager' }} | BPR XYZ</title>
    ...
    <aside id="sidebar"
        ...
        <nav class="mt-6 px-2 space-y-1">
            <a href="{{ route('manager.dashboard') }}"
                class="flex items-center px-4 py-3 rounded-lg transition-colors group {{ request()->routeIs('manager.dashboard') ? 'bg-[#1b263b] text-white' : 'text-gray-300 hover:bg-[#1b263b] hover:text-white' }}">
                <i class="fa-solid fa-house w-6 text-center mr-2 text-sm"></i>
                <span class="font-medium">Beranda</span>
            </a>

            <a href="{{ route('manager.rekomendasi.index') }}"
                class="flex items-center px-4 py-3 rounded-lg transition-colors group {{ request()->routeIs('manager.rekomendasi.index', 'manager.rekomendasi.show') ? 'bg-[#1b263b] text-white' : 'text-gray-300 hover:bg-[#1b263b] hover:text-white' }}">
            ...
            <a href="{{ route('manager.rekomendasi.riwayat') }}"
                class="flex items-center px-4 py-3 rounded-lg transition-colors group {{ request()->routeIs('manager.rekomendasi.riwayat', 'manager.rekomendasi.detail') ? 'bg-[#1b263b] text-white' : 'text-gray-300 hover:bg-[#1b263b] hover:text-white' }}">
            ...
            <a href="{{ route('manager.angsuran.index') }}"
                class="flex items-center px-4 py-3 rounded-lg transition-colors group {{ request()->routeIs('manager.angsuran.*') ? 'bg-[#1b263b] text-white' : 'text-gray-300 hover:bg-[#1b263b] hover:text-white' }}">
                <i class="fa-solid fa-clock-rotate-left w-6 text-center mr-2 text-sm"></i>
                <span class="font-medium">Kredit & Angsuran</span>
            </a>

            @php
                $isLaporanActive = request()->routeIs('manager.laporan.*');
            @endphp
            ...
            <div>
                ...
                <div id="laporanMenu" class="space-y-1 mt-1 {{ $isLaporanActive ? 'block' : 'hidden' }}">
                    ...
                    <a href="{{ route('manager.laporan.analisis') }}"
                        class="block pl-12 pr-4 py-2 rounded-lg transition-colors {{ request()->routeIs('manager.laporan.analisis') ? 'text-white bg-[#26354f]' : 'text-gray-400 hover:text-white hover:bg-[#26354f]' }}">
                        Analisis Kredit
                    </a>
