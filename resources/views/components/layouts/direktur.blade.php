...
<head>
    ...
    <title>{{ $title ?? 'Dashboard Direktur' }} | BPR Parinama Simfoni Indonesia</title>
    ...
    <aside id="sidebar"
        ...
        <nav class="mt-6 px-2 space-y-1">
            ...
            <a href="{{ route('direktur.dashboard') }}"
                class="flex items-center px-4 py-3 rounded-lg transition-colors group {{ request()->routeIs('direktur.dashboard') ? 'bg-[#1b263b] text-white' : 'text-gray-300 hover:bg-[#1b263b] hover:text-white' }}">
                <i class="fa-solid fa-house w-6 text-center mr-2 text-sm"></i>
                <span class="font-medium">Beranda</span>
            </a>

            <a href="{{ route('direktur.persetujuan.index') }}"
                class="flex items-center px-4 py-3 rounded-lg transition-colors group {{ request()->routeIs('direktur.persetujuan.index') ? 'bg-[#1b263b] text-white' : 'text-gray-300 hover:bg-[#1b263b] hover:text-white' }}">
            ...
            <div>
                ...
                <div id="laporanMenu" class="space-y-1 mt-1 {{ $isLaporanActive ? 'block' : 'hidden' }}">
                    ...
                    <a href="{{ route('direktur.laporan.nasabah') }}"
                    ...
                    <a href="{{ route('direktur.laporan.analisis') }}"
                        class="block pl-12 pr-4 py-2 rounded-lg transition-colors {{ request()->routeIs('direktur.laporan.analisis') ? 'text-white bg-[#26354f]' : 'text-gray-400 hover:text-white hover:bg-[#26354f]' }}">
                        Analisis Kredit
                    </a>

                    <a href="{{ route('direktur.laporan.monitoring') }}"
                        class="block pl-12 pr-4 py-2 rounded-lg transition-colors {{ request()->routeIs('direktur.laporan.monitoring') ? 'text-white bg-[#26354f]' : 'text-gray-400 hover:text-white hover:bg-[#26354f]' }}">
                        Monitoring Angsuran
                    </a>

                    <a href="{{ route('direktur.laporan.rekapitulasi') }}"
                        class="block pl-12 pr-4 py-2 rounded-lg transition-colors {{ request()->routeIs('direktur.laporan.rekapitulasi') ? 'text-white bg-[#26354f]' : 'text-gray-400 hover:text-white hover:bg-[#26354f]' }}">
                        Rekapitulasi
                    </a>
                </div>
            </div>
