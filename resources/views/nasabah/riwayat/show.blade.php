<x-layouts.nasabah :title="'Detail Pengajuan'">
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold">Detail Pengajuan #{{ $application->id }}</h1>
            ...
            <div class="bg-white rounded-2xl shadow-md p-6">
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <dd class="font-medium">{{ $application->creditFacility->nama_fasilitas }}</dd>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <dd class="font-medium text-lg text-green-600">Rp {{ number_format($application->jumlah_pinjaman, 0, ',', '.') }}</dd>
                    </div>
                    <div>
            ...
            <div class="bg-white rounded-2xl shadow-md p-6">
                ...
                <div class="text-center py-4">
                    ...
                    @else
                        ...
                        <a href="{{ route('pengajuan.step1') }}" class="mt-4 inline-block bg-yellow-500 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-yellow-600">Lanjutkan Pengisian</a>
                    @endif
                </div>
                ...
                <div class="mt-6 pt-6 border-t">
                    <div class="flex justify-between text-sm mb-2">
                        ...
                        <span class="font-medium">{{ $application->submitted_at ? \Carbon\Carbon::parse($application->submitted_at)->format('d M Y') : '-' }}</span>
                    </div>
                    @if($application->approved_at)
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Disetujui pada:</span>
                        <span class="font-medium">{{ \Carbon\Carbon::parse($application->approved_at)->format('d M Y') }}</span>
                    </div>
                    @endif
                </div>
            </div>
