...
<x-dynamic-component :component="$layout" :title="'Kartu Angsuran'">
            ...
            <table class="w-full text-sm text-left text-gray-600">
                <thead class="bg-gray-50 text-gray-700 uppercase font-semibold">
                    <tr>
                        ...
                        <th class="px-6 py-4">Reversal Info</th> {{-- Kolom Baru --}}
                        <th class="px-6 py-4 text-center" style='{{ $displayAdminAction }}'>Aksi</th>
                        <th class="px-6 py-4 text-center" style='{{ $displayDirekturAction }}'>Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @foreach ($payments as $pay)
                        @php
                            ...
                            $rowClass = $pay->reversal_date ? 'bg-red-50 italic' : 
                                        ($pay->status_pembayaran == 'Paid' ? 'bg-green-50' : 'hover:bg-gray-50');
                        @endphp
                        ...
                        <tr class="{{ $rowClass }}">
                            ...
                            {{-- Data Realisasi --}}
                            <td class="px-6 py-4">{{ $pay->tanggal_bayar ? $pay->tanggal_bayar->format('d M Y') : '-' }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                @if ($pay->reversal_date)
                                    <span class="bg-gray-800 text-white text-xs font-bold px-2 py-1 rounded">DIBATALKAN</span>
                                ...
                                @elseif($pay->status_pembayaran == 'Partial')
                                    <span class="bg-yellow-100 text-yellow-800 text-xs font-bold px-2 py-1 rounded">NYICIL</span>
                                @else
                                    <span class="bg-gray-100 text-gray-800 text-xs font-bold px-2 py-1 rounded">BELUM</span>
                                @endif
                            </td>
                            
                            {{-- Kolom Reversal Info (BARU) --}}
                            <td class="px-6 py-4 text-xs">
                                @if($pay->reversal_date)
                                    <span class="text-red-700 font-medium">({{ $pay->reversal_date->format('d/m/y') }})</span>
                            ...
                            {{-- KOLOM AKSI ADMIN --}}
                            <td class="px-6 py-4 text-center" style='{{ $displayAdminAction }}'>
                            ...
                            {{-- KOLOM AKSI DIREKTUR --}}
                            <td class="px-6 py-4 text-center" style='{{ $displayDirekturAction }}'>
                                @if (!$pay->reversal_date && ($pay->status_pembayaran == 'Paid' || $pay->status_pembayaran == 'Partial'))
                                    <button
                                        onclick="openReversalModal({{ $pay->id }})"
                                        class="bg-red-600 text-white px-3 py-1 rounded text-xs hover:bg-red-700">
                                        Reversal
                                    </button>
                                @elseif($pay->reversal_date)
                                    <button disabled class="text-xs text-red-700 bg-red-100 px-3 py-1 rounded">Sudah Dibatalkan</button>
                                @else
                                    <button disabled class="text-gray-400 cursor-not-allowed text-xs">Belum Dibayar</button>
                                @endif
                            </td>
                        </tr>
                    @endforeach
    ...
    <div id="paymentModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg w-full p-6 relative z-10">
                <h3 class="text-lg font-bold text-gray-900 mb-4">Input Pembayaran Angsuran Ke-<span id="modalAngsuranKe"></span></h3>
                ...
                <form id="paymentForm" method="POST">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="date" name="tanggal_bayar" value="{{ date('Y-m-d') }}" required class="w-full border-gray-300 rounded-lg">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Jumlah Bayar (Rp)</label>
                            <input type="number" name="jumlah_bayar" id="modalJumlahBayar" required class="w-full border-gray-300 rounded-lg font-bold text-green-700">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Denda (Jika Telat)</label>
                            <input type="number" name="denda" value="0" class="w-full border-gray-300 rounded-lg text-red-600">
                        </div>
                        <div>
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        <button type="button" onclick="closePaymentModal()" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-200">Batal</button>
    ...
    <div id="reversalModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onclick="closeReversalModal()"></div>
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg w-full p-6 relative z-10">
                ...
                <form id="reversalForm" method="POST">
                    ...
                    <div class="space-y-4">
                        <p class="text-sm text-gray-700 mb-4">Anda akan membatalkan transaksi ini. Status akan di-reset menjadi BELUM BAYAR.</p>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Tanggal Reversal</label>
                            <input type="date" name="reversal_date" value="{{ date('Y-m-d') }}" required class="w-full border-gray-300 rounded-lg">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Catatan Reversal (Wajib)</label>
                            <textarea name="reversal_note" rows="3" required placeholder="Jelaskan alasan pembatalan (Contoh: Salah input nominal / Transfer fiktif)" class="w-full border-gray-300 rounded-lg"></textarea>
                        </div>
                    </div>

                    <div class="mt-6 flex justify-end gap-3">
                        <button type="button" onclick="closeReversalModal()" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-200">Tutup</button>
                        <button type="submit" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700">Ya, Batalkan Transaksi</button>
                    </div>
                </form>
            </div>
        </div>
